...
*-----------------------------------------------------------------------------------*/
        ...
        public static bool createClient(bool useDynamoDBLocal)
        {
            if (useDynamoDBLocal)
            {
                ...
                Message = String.Format("  -- Setting up a DynamoDB-Local client (DynamoDB Local seems to be running)");
                Console.WriteLine(Message);
                ...
                {
                    Message = String.Format("     FAILED to create a DynamoDBLocal client; " + ex.Message);
                    Console.WriteLine(Message);
                    operationFailed = true;
                    return false;
                }
